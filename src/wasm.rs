...
impl WasmPong {
    ...
    /// Reset environment, returns initial state [ball_x, ball_y, left_y, right_y, left_score, right_score]
    #[wasm_bindgen]
    pub fn reset(&mut self) -> Vec<f32> {
        self.env.reset();
        self.episode += 1;
    ...
    /// Returns [ball_x, ball_y, left_y, right_y, left_score, right_score, reward, terminated, truncated]
    #[wasm_bindgen]
    pub fn step(&mut self, action: i32) -> Vec<f32> {
        let result = self.env.step(action as i64);
        self.episode_steps += 1;
    ...
    /// Current rendering state [ball_x, ball_y, left_y, right_y, left_score, right_score]
    #[wasm_bindgen]
    pub fn get_state(&self) -> Vec<f32> {
        self.env.get_state().to_vec()
    }
