#!/usr/bin/env bash
# Run hyperparameter optimization for Snake on GPU machine
#
# Usage (on GPU machine):
#   ./scripts/gpu-optimize-snake.sh [--trials N]
#
# Example:
#   ./scripts/gpu-optimize-snake.sh --trials 50
...
# Skip cargo clean to avoid OOM on machines with limited RAM
cargo +nightly build --example optimize_snake --release

# Create log file with timestamp
LOG_FILE="snake_optimization_$(date +%Y%m%d_%H%M%S).log"

echo ""
echo "🚀 Starting Snake hyperparameter optimization"
echo "📊 Trials: $TRIALS"
...
cargo +nightly run --example optimize_snake --release -- --trials "$TRIALS" 2>&1 | tee "$LOG_FILE"
