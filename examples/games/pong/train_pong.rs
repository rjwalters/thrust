//! Train PPO on Pong — agent (left paddle) vs rule-based opponent (right paddle).
//!
//! # Usage
//!
//! ```bash
...
fn main() -> Result<()> {
    ...
    tracing::info!("obs_dim={obs_dim}, action_dim={action_dim}, num_envs={NUM_ENVS}, steps={NUM_STEPS}");
    ...
    for update in 0..num_updates {
        ...
        if update % 20 == 0 {
            ...
            let avg = if episodes > 0 { steps as f64 / episodes as f64 } else { 0.0 };
            tracing::info!(
                "Update {}/{} | Steps: {} | Episodes: {} | Avg: {:.1} | Loss: {:.3} | Policy: {:.3} | Value: {:.3} | Entropy: {:.3} | ExpVar: {:.3}",
                update + 1, num_updates, steps, episodes, avg,
                stats.total_loss, stats.policy_loss, stats.value_loss,
                stats.entropy, stats.explained_var,
            );
        }
    ...
    tracing::info!("✅ Training complete! Steps={} Episodes={}", trainer.total_steps(), trainer.total_episodes());
